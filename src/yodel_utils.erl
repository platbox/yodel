...
check_success({ok, V1}, _) -> V1;
check_success({ok, V1, V2}, _) -> {V1, V2};
check_success({ok, V1, V2, V3}, _) -> {V1, V2, V3};
check_success({error, R1}, Error) -> throw({Error, R1});
check_success({error, R1, R2}, Error) -> throw({Error, R1, R2});
check_success({error, R1, R2, R3}, Error) -> throw({Error, R1, R2, R3}).
...
log(Level, Str, Args) when is_atom(Level) ->
    ...
    case should_log(LogLevel, Level) of
        true ->
            LogStr = io_lib:format(log_prefix(Level) ++ Str ++ "~n", Args),
            io:put_chars(log_stream(Level), LogStr);
        false ->
            ok
    end.

log_stream(error) -> standard_error;
log_stream(warn ) -> standard_error;
log_stream(_    ) -> standard_io   .
